...
export function createBoard(
  ...
  function findCell(bet: Bet): HTMLElement | null {
    if (bet.type === 'straight') {
      ...
      if (num === 0) {
        return table.querySelector('.zero-cell:not(.double-zero-cell)') as HTMLElement | null
      }
      if (num === DOUBLE_ZERO) {
        return table.querySelector('.double-zero-cell') as HTMLElement | null
      }
      return grid.querySelector(`[data-num="${num}"]`) as HTMLElement | null
    }
